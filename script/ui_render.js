...
let defaultString_1 = "tapclank pre alpha v1.01.1 2025 by lumovalley labs";
let defaultString_2 = "press g to open or close the editor menu";
...
function renderText() {
	for (o = 0; o < stringToRender.length; o++) {
		...
		if (characterToRender == 32) { textX+=13; continue; } // space
		if (highlightBackOfText) { 
			screen.drawImage(transparentBlack, textX-4, textY-2, stringToRender.length * (textX_size+4), textY_size+4); 
...
// To draw the menu item bar ( the 4 materials ) and the draw + erase slots
function drawMenuItemBar() {
	
	// First, we clear the UI buffer
	screenBuffer2.clearRect(4, 150, 120, 270);
	...
	if (cursorOverWhichMenuBarSlot == 1) {
		screen.drawImage(transparentCursor, 10+itemMenuBarSlotsAdditionalPositions[1], 150, 34, 34);
		drawItemMenuBarSlotHoverText();
	} else if (cursorOverWhichMenuBarSlot == 2) {
		screen.drawImage(transparentCursor, 10+itemMenuBarSlotsAdditionalPositions[2], 190, 34, 34);
		drawItemMenuBarSlotHoverText();
	} else if (cursorOverWhichMenuBarSlot == 3) {
		screen.drawImage(transparentCursor, 10+itemMenuBarSlotsAdditionalPositions[3], 230, 34, 34);
		drawItemMenuBarSlotHoverText();
	} else if (cursorOverWhichMenuBarSlot == 4) {
		screen.drawImage(transparentCursor, 10+itemMenuBarSlotsAdditionalPositions[4], 270, 34, 34);
		drawItemMenuBarSlotHoverText();
	} else if (cursorOverWhichMenuBarSlot == 5) {
		screen.drawImage(transparentCursor, 10+itemMenuBarSlotsAdditionalPositions[5], 320, 34, 34);
		drawItemMenuBarSlotHoverText();
	} else if (cursorOverWhichMenuBarSlot == 6) {
		screen.drawImage(transparentCursor, 10+itemMenuBarSlotsAdditionalPositions[6], 360, 34, 34);
		drawItemMenuBarSlotHoverText();
	}
